...
class TestFunctionalDnaasm(unittest.TestCase):
    ...
    def clickCssLink(self, ident, interval=0.1, maxTime=2):
        ...
        self.assertGreaterEqual(len(link), 1,
                                "Cannot find link with ident='{css}' in {brow}".format(css=ident, brow='browser'))
        link.first.click()
    ...
    def test03StartPageTranslations(self):
        ...
        self.assertEqual(self.browser.find_by_id('loginAsGuestButton').first['title'],
                         u"Kliknij, aby zalogować się do aplikacji jako gość")
        self.assertEqual(self.browser.find_by_id('showLoginWindowButton').first.text, u'Zaloguj się')
        self.assertEqual(self.browser.find_by_id('showLoginWindowButton').first['title'],
                         u"Kliknij, aby zalogować się do aplikacji")
        self.assertEqual(self.browser.find_by_id('showNewUserWindowButton').first.text, u'Dodaj użytkownika')
        self.assertEqual(self.browser.find_by_id('showNewUserWindowButton').first['title'],
                         u"Kliknij, aby dodać nowego użytkownika")
        self.assertEqual(self.browser.find_by_id('showHelpWindowButton').first.text, u'Pomoc')
        self.assertEqual(self.browser.find_by_id('showHelpWindowButton').first['title'],
                         u"Kliknij, aby zobaczyć pomoc dla aplikacji")
        self.assertEqual(self.browser.find_by_id('a_lang_en').first['title'],
                         u"Kliknij, aby zmienić język na angielski")
